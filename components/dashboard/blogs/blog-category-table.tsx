...
import { BlogCategoryActions } from "@/components/dashboard/blogs/blog-category-actions"
import { formatDateTime } from "@/components/dashboard/products/product-utils"
import { listAdminBlogCategories, type BlogCategory, type BlogCategoryStatus } from "@/lib/api/blogs"
  ...
  const columns = useMemo<ColumnDef<BlogCategory>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <BlogCategoryStatusBadge status={row.original.status} />
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => <BlogCategoryActions category={row.original} />,
      },
    ],
      ...
      <section className="grid gap-4 rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5 lg:grid-cols-[1fr_auto] lg:items-center">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
            Organize blog posts into browsable groups for the storefront.
          </p>
        </div>
...
function CategoryIdentity({ category }: { category: BlogCategory }) {
  return (
...
function mobileRow(row: Row<BlogCategory>) {
    ...
    >
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-2">
        ...
        <div className="shrink-0">
          <BlogCategoryActions category={category} />
        </div>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <BlogCategoryStatusBadge status={category.status} />
      </div>
...
function parseStatus(value: string | null): BlogCategoryStatus | undefined {
