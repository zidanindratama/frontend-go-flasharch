...
import { BlogCategoryStatusBadge } from "@/components/dashboard/blogs/blog-badges"
...
type BlogCategoryFormProps =
  ...
  | { mode: "edit"; category: BlogCategory }

export function BlogCategoryForm(props: BlogCategoryFormProps) {
  return props.mode === "create" ? (
    <CreateBlogCategoryForm />
  ) : (
    <EditBlogCategoryForm category={props.category} />
  )
}

function CreateBlogCategoryForm() {
  const router = useRouter()
  const queryClient = useQueryClient()

  const form = useForm<BlogCategoryCreateValues>({
  ...
  const createMutation = useMutation({
    mutationFn: (input: BlogCategoryCreateValues) =>
      createBlogCategory(input),
    onSuccess: async () => {
  ...
  function onSubmit(input: BlogCategoryCreateValues) {
  ...
  return (
    <FormShell
      title="Add category"
      description="Create a new blog category for storefront navigation."
      preview={
        <CategoryPreview
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-5">
        <section className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Name, URL slug, and description for this category.
          </p>
          <div className="mt-5 grid gap-4">
            <Field>
              ...
              <Input
                id="name"
                placeholder="e.g. Shopping Guides"
            ...
            <Field>
              ...
              <Select
                value={values.status}
                onValueChange={(v) =>
                  form.setValue("status", v as BlogCategoryStatus, {
                    shouldValidate: true,
                  })
                }
              >
...
function EditBlogCategoryForm({ category }: { category: BlogCategory }) {
  const router = useRouter()
  const queryClient = useQueryClient()

  const form = useForm<BlogCategoryEditValues>({
    resolver: zodResolver(blogCategoryEditSchema),
    defaultValues: {
      slug: category.slug,
      name: category.name,
  ...
  const updateMutation = useMutation({
    mutationFn: (input: BlogCategoryEditValues) =>
      updateBlogCategory(category.id, input),
    onSuccess: async () => {
      const { toast } = await import("sonner")
  ...
  function onSubmit(input: BlogCategoryEditValues) {
  ...
  return (
    <FormShell
      title="Edit category"
      description="Update category name, description, and storefront visibility."
      preview={
        <CategoryPreview
          name={values.name || category.name}
          slug={values.slug || category.slug}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <BlogCategoryStatusBadge status={status} />
      </div>
      {createdAt && (
