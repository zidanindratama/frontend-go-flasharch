...
function StockAlertCard({
  ...
  const handleRemove = useCallback(
    ...
    [confirmRemove, onRemove, item.product.slug],
  )
      ...
      >
        ...
        <div className="flex shrink-0 items-start">
          <motion.button
            ...
            disabled={isRemoving}
            aria-label={confirmRemove ? "Confirm remove" : "Remove flash sale alert"}
            className={cn(
          ...
          >
            {isRemoving ? (
              ...
              <Bell className="h-4 w-4 fill-[#FF6600] text-[#FF6600] transition-transform" />
            )}
          </motion.button>
        </div>
      </Link>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Bell className="h-7 w-7 text-[#FF6600]/60" />
        </motion.div>
      </motion.div>
      <h2 className="text-base font-semibold tracking-tight">
        No flash sale alerts yet
      </h2>
      <p className="mt-1.5 max-w-xs text-sm leading-relaxed text-muted-foreground">
        Get notified when products you follow join an upcoming flash sale.
      </p>
...
export function AccountStockAlerts() {
  ...
  const removeMutation = useMutation({
    ...
    onMutate: async (slug) => {
      await queryClient.cancelQueries({ queryKey: ["account.stockAlerts"] })
      const previous = queryClient.getQueryData<StockAlert[]>(["account.stockAlerts"])
      queryClient.setQueryData<StockAlert[]>(["account.stockAlerts"], (old) => {
        if (!old) return old
        return old.filter((item) => item.product.slug !== slug)
      })
      return { previous }
    },
    onSuccess: async () => {
    ...
    onError: async (_err, _slug, context) => {
      if (context?.previous) {
        queryClient.setQueryData(["account.stockAlerts"], context.previous)
      }
      const { toast } = await import("sonner")
      toast.error("Failed to remove flash sale alert")
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["account.stockAlerts"] })
    },
  })
            ...
            {items.map((item) => (
              ...
              >
                <StockAlertCard
                  ...
                  isRemoving={removeMutation.isPending && removeMutation.variables === item.product.slug}
                />
              </motion.div>
            ))}
          </AnimatePresence>
