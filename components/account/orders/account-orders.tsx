...
import { CreditCard, Loader2, Package, PackageOpen, ShoppingBag } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { OrderStatusBadge } from "@/components/common/order-status-badge";
import { getBuyerOrders, type BuyerOrderRow } from "@/lib/api/account";
...
function OrderCard({ order }: { order: BuyerOrderRow }) {
  ...
  const isUnpaid = ["pending", "pending_payment", "waiting_payment"].includes(
    order.status,
  );
  ...
  return (
    <Link href={`/account/orders/${order.id}`}>
      <motion.div
        ...
        className="rounded-xl bg-card p-4 ring-1 ring-foreground/10 transition-colors hover:bg-accent/50"
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex items-center justify-between gap-3 sm:block sm:text-right">
            ...
            <OrderStatusBadge status={order.status} className="sm:mt-1" />
          </div>
        </div>
...
export function AccountOrders() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "account.orders",
      { page, perPage, sort: "created_at", order: "desc" },
    ],
    queryFn: async () => {
