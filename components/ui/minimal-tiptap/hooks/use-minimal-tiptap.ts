    ...
    onDrop: async (editor, files, pos) => {
      const uploaded = await Promise.all(
        files.map(async (file) => {
          ...
          return { type: "image" as const, attrs: { id: randomId(), src, alt: file.name, title: file.name, fileName: file.name } }
        }),
      )
      editor.chain().focus().insertContentAt(pos, uploaded).run()
    },
      ...
      const images = await Promise.all(
        files.map(async (file) => {
          const src = uploader
            ? await uploader(file)
            : URL.createObjectURL(file)
          return { src, alt: file.name, title: file.name }
        }),
      )
      editor.commands.setImages(images)
    },
    onValidationError: (errors) => {
      errors.forEach((error) => {
        toast.error("Image validation error", {
