...
import { createCartCheckout, createDirectFlashSaleCheckout, getCheckout } from "@/lib/api/checkout"
...
export function useFlashSaleCheckout() {
  return useMutation({
    mutationFn: ({
      flashSaleItemId,
      addressId,
...
export function useCheckoutFlow(cartId: string | null) {
  ...
  const startCheckout = useCallback(
    async (addressId?: string, shippingCost?: number, carrier?: string) => {
      if (!cartId) return
      setPhase("checkout")
...
export function useFlashSaleCheckoutFlow() {
  const router = useRouter()
  const [checkoutId, setCheckoutId] = useState<string | null>(null)
  const [paymentId, setPaymentId] = useState<string | null>(null)
  const [phase, setPhase] = useState<
  ...
  const flashSaleCheckoutMutation = useFlashSaleCheckout()
  const { checkout, status } = useCheckoutPolling(
    checkoutId,
    phase === "polling",
  )
  ...
  const startCheckout = useCallback(
    async (
      ...
      try {
        const response = await flashSaleCheckoutMutation.mutateAsync({
          flashSaleItemId,
          addressId,
          shippingCost,
          carrier,
  ...
  return {
    ...
    checkout,
    startCheckout,
    isStarting: flashSaleCheckoutMutation.isPending,
    isRedirecting: paymentCheckoutMutation.isPending,
  }
}
